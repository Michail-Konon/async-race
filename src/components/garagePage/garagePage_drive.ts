import { getCarsAPI, startMotorAPI, driveMotorAPI, stopMotorAPI, getCarAPI } from '../api/api_garage';
...
import { numberPage } from './garagePage';
import { DescriptionCar } from '../carGen/carGeneration';

const btnResetRace = <HTMLButtonElement>document.querySelector('.btn_race-stop');
const btnRace = <HTMLButtonElement>document.querySelector('.btn_race-start');
const btnStartRace = <HTMLButtonElement>document.querySelector('.btn_race-start');
const btnStopRace = <HTMLButtonElement>document.querySelector('.btn_race-stop');
...
function addWinner(carWinner: HTMLElement, timeWinner: number) {
  ...
  getCarAPI(idWinner).then((arr) => {
    nameWinner = arr.name;
    noticeWinner.innerHTML = `${nameWinner} finish first (${timeWin}s) `;
  });
